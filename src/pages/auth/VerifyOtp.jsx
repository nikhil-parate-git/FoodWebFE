...
export default function VerifyOtp() {
  ...
  const from = location.state?.from || "forgot"; // "forgot" or "signup"
  ...
  const handleVerify = async () => {
    ...
    setLoading(false);
    // On success: go to reset-password if came from forgot, else to dashboard
    if (from === "forgot") {
    ...
    <div className="min-h-screen flex bg-[#0d0905] overflow-hidden relative font-['Nunito']">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-10">
        <div className="w-full max-w-[460px]">
          ...
          <button
            type="button"
            onClick={() => navigate(from === "forgot" ? "/forgot-password" : "/login")}
          ...
          <div className="animate-fadeUp delay-1 mb-8">
            ...
            <h2 className="text-[1.6rem] font-black text-white tracking-tight">Verify OTP 🔐</h2>
            <p className="text-sm text-white/35 mt-1.5">
              Code sent to <span className="text-white/60 font-bold">{maskedEmail}</span>
            </p>
          </div>
          ...
          <div className="animate-fadeUp delay-3 mb-2">
            <label className="text-[14.5px] font-black text-white mb-4 block text-center">Enter 6-digit OTP</label>
            ...
            {error && <p className="text-red-400 text-[11px] mt-3 text-center">{error}</p>}
          </div>
